...
try(Connection con=DriverManager.getConnection(url,user,psswd)){
	PreparedStatement stmt=con.prepareStatement("Select Password from Seller where UserName=?");
	...
	if(rs.next()){
		if(rs.getString(1).equals(password)){
			...
			out.println("<script>alert('Incorrect Password')</script>"+rs.getString(1)+password);
		}
	}
	else{
		out.println("<script>alert('User Name does not exist.Please Go back and Register/ Write correct User Name ')</script>");
	}
